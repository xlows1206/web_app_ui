...
> 最后审计时间：2026-04-05 03:50 | 审计工具：ts-prune + grep import 双重验证
...
| **AssistantPanel** | 1 | `isMinimized`, `onMinimize`, `onExpand` | 助手面板容器，含最小化占位态 | `components/views/SessionActiveView` |
...
| **GlassActionMenu** | 1 | `actions`, `adminName`, `createdAt` | 沉浸式毛玻璃下拉菜单，带元数据页眉 | `ui/ProjectCard` |
| **CaseDetailModal** | 1 | `caseData`, `typeLabel`, `onApply` | 案例详情展示弹窗 | `components/views/ProjectSandboxView` |
...
| **LiveViewPanel** | 1 | `isFullscreen`, `liveViewState`, `statusLabel` | 实时视图面板容器，含全屏逻辑与呼吸动画 | `components/views/SessionActiveView` |
...
| **SlidingTabs** | 2 | `tabs`, `activeTab`, `onChange` | Pill 式滑动切换标签页，带平滑动画 | `ui/SessionWorkspaceSidebar`, `ui/ProjectWorkspaceSidebar`* |
...
*Last Updated: 2026-04-05 03:50 | Audited via: ts-prune + grep double-check*
